/* 
	Build Command: gcc APCF.c ../../../modules/filters.c ../../../modules/wav.c ../../../modules/fft.c ../../../modules/complex.c -lm	
	Run Command: ./a.out
*/
...
void main() {
	...
	for (int i=0; i<output_info.num_samples; ++i)
	{
		step_ap_comb_filter(APCF, input_fdata[i], &output);
	...
	APCF2 = init_ap_comb_filter(delay_length, b0, am);

	for (int i=output_info.num_samples; i<(output_info.num_samples*output_info.num_channels); ++i)
	{
		step_ap_comb_filter(APCF2, input_fdata[i], &output);
		output_fdata[i] = output;
	...
	char* output_file = "APCF_M4000_b005_am-05.wav";
