/* 
	Build Command: gcc SR.c ../../../modules/filters.c ../../../modules/wav.c ../../../modules/fft.c ../../../modules/complex.c -lm	
	Run Command: ./a.out
*/
...
void main() {
	...
	for (int i=0; i<output_info.num_samples; ++i)
	{
		step_shroeder_reverberator(SR, input_fdata[i], &output);
	...
	SR2 = init_shroeder_reverberator(ap_delay_lengths, fb_delay_lengths, ap_gain, fb_gains); 

	for (int i=output_info.num_samples; i<(output_info.num_samples*output_info.num_channels); ++i)
	{
		step_shroeder_reverberator(SR2, input_fdata[i], &output);
		output_fdata[i] = output;
	...
	char* output_file = "shroeder_reverberator.wav";
