/* 
	Build Command: gcc FBCF_DeChaka.c ../../modules/filters.c ../../modules/wav.c -lm	
	Run Command: ./a.out
*/
...
void main() {
	...
	for (int i=0; i<output_info.num_samples; ++i)
	{
		step_fb_comb_filter(FBCF, input_fdata[i], &output);
	...
	FBCF2 = init_fb_comb_filter(delay_length, b0, am);

	for (int i=output_info.num_samples; i<(output_info.num_samples*output_info.num_channels); ++i)
	{
		step_fb_comb_filter(FBCF2, input_fdata[i], &output);
		output_fdata[i] = output;
